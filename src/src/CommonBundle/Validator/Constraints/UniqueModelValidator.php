 ...
 * можем проверить уникальность сущности, данные о которой содержаться в объекте-команде.
 */
class UniqueModelValidator extends ConstraintValidator
{
    ...
    public function validate($model, Constraint $constraint)
    {
        if (!$constraint instanceof UniqueModel) {
            throw new UnexpectedTypeException($constraint, __NAMESPACE__ . '\UniqueModel');
        }
        ...
        if (1 === count($result) && $constraint->excludedIdField) {
            $resultEntity = $result instanceof \Iterator ? $result->current() : current($result);
            $excludedIdValue = $this->propertyAccessor->getValue($model, $constraint->excludedIdField);

            if (count($classMetadata->getIdentifierFieldNames()) > 1) {
                $idValue = $classMetadata->getIdentifierValues($resultEntity);
            } else {
                $idValue = current($classMetadata->getIdentifierValues($resultEntity));
            }

            if ($excludedIdValue === $idValue
                || (is_object($idValue) && (string) $excludedIdValue === (string) $idValue)) {
                return;
            }
        }
